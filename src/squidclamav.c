...
void generate_alert(char* url, char* source, char* user, char* virus);
...
int add_pattern(char *s, int level)
{
    ...
    /* Path to alert program */
    if(strcmp(type, "alert") == 0) {
        alert = (char *) malloc (sizeof (char) * LOW_BUFF);
        if(alert == NULL) {
            fprintf(stderr, "unable to allocate memory in add_to_patterns()\n");
            free(type);
            free(first);
...
void generate_alert(char* url, char* source, char* user, char* virus) {
    if(alert != NULL) {
        char *prog = (char *) malloc(sizeof(char) * MAX_URL_SIZE);
        snprintf(prog, MAX_URL_SIZE, "%s '%s' '%s' '%s' '%s'", alert, url, source, user, virus);
        system(prog);
        free(prog);
    }
}
...
void generate_response_page(ci_request_t *req, av_req_data_t *data)
{
    ...
    generate_alert(data->url, data->clientip, data->user, data->malware);
...
void generate_template_page(ci_request_t *req, av_req_data_t *data)
{
    ...
    ci_http_response_add_header(req, "Server: Zorus");
